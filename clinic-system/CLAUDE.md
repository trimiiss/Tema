...
- **`get_available_slots` does not filter out the past, and must not start.** It answers "does this doctor work this slot?", which stays true for a morning that has already happened — what a receptionist recording an earlier walk-in through the manual form needs. "Can a visitor still book it?" is a different question, answered by `drop_past_slots` / `earliest_bookable_instant()` (now + `MIN_BOOKING_LEAD_MINUTES`) on the visitor-facing surfaces only: `GET /public/slots`, the booking agent's `list_available_slots` and `find_earliest_slot`, and `propose_booking`, which refuses a past time outright and hands back this doctor's real next openings. Both public orchestrators put the current clinic **time**, not just the date, in the agent's context, because a model given only the date offers 09:00 at four in the afternoon.
